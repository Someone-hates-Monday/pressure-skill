...
> **实地反馈**：用户说**已发出建议、对方真实回复、和预测不一样、澄清了模糊话**时，**再读取** `references/counterparty-feedback-loop.md`，用 `counterparty_cli.py feedback` 把偏差写入 `outcomes.jsonl` 并校准 `profile.json`。  
...
| **已发出话术 + 真实对方回复/澄清** | 读 **`counterparty-feedback-loop.md`** → 偏差分析 → `feedback` CLI；完成后可再进 §5～10 新会诊。 |
