...
2. **隐私**：默认不落盘；请**脱敏**。  
3. 流程：**先一起把「这个人」画像搭起来 → 再问你此刻目的 → 再选怎么应对（含可选回怼）**。  
...
## 8. 是否运行 `bundle_local.py`

问：要不要在 **`压力.skill` 仓库根** 跑脚本出 JSON（含 `readiness`、`warnings`）？**要 / 不用**。
...
> 你现在最需要我**优先**给你哪一类？（可多项）**A** 直接回答你的具体问题 **B** 对方意图推测 **C** 可发送话术/邮件片段 **D** `readiness`/信息缺口解读 **E** 风险与禁区 —— 也可用**自己的一句话**说明优先级。
...
4. **对方意图推测** + **置信度**——须遵守 **`readiness.portrait_depth_hint`**（若未跑脚本，Agent 自行声明当前推断处于 minimal/standard/deep 哪档及理由）。**`deep`** 时可引用机制语言（激励、升级链、面子），但**每条须能指回**侧写/对话/场外中的原词或事实；**`minimal`** 时禁止高置信读心，以澄清问句为主。若 **`discipline_subfield` 或 `artefact_preferences_notes` 为空** 或与对方原话可能冲突，须列出 **2～3 条互斥解读**（例：要形式化推导 vs 要复现实验 vs 要系统叙事），并说明缺哪条信息会误判。  
5. **回复建议** 2～3 条——**必须显式融入** `user_leverage_notes` 与 `extra_context_notes` 中的事实，禁止泛泛鸡汤  
...
从首条消息抽取：关系、侧写/对话、筹码、场外、**目的**、场景与正文 → 直接 §8 或 §10。
